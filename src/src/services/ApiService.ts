...
export class ApiService {
  ...
  public lnDeposit(): Promise<any> {
    const data = {
      amountSats: 1000, description: 'testing', network: 'lightning'
    }

    return this.authService.getUser().then(user => {
      if (user && user.access_token) {
        return this._postApi(user.access_token, 'deposits', data).catch(error => {
          if (error.response.status === 401) {
            return this.authService.renewToken().then(renewedUser => {
              return this._postApi(renewedUser.access_token, 'deposits', data);
            });
          }
          throw error;
        });
  ...
  public listWithdrawals(): Promise<any> {
    return this.authService.getUser().then(user => {
      if (user && user.access_token) {
        return this._getApi(user.access_token, 'withdrawals').catch(error => {
          if (error.response.status === 401) {
            return this.authService.renewToken().then(renewedUser => {
              return this._getApi(renewedUser.access_token, 'withdrawals');
            });
          }
          throw error;
        });
